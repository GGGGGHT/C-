...
void JNICALL InitCallback(jvmtiEnv *jvmti_env,
     JNIEnv* jni_env,
     jthread thread) {
...
void JNICALL ThreadStartCallback(jvmtiEnv *jvmti_env,
     JNIEnv* jni_env,
     jthread thread) {
         threadCount ++;
         char buffer[50];
        int n = sprintf(buffer, "start a new Thread, current has %d alive thread.", threadCount);
        printf("[%s] is a string %d char long\n",buffer,n);
        fflush(stdout);
}

void JNICALL ThreadEndCallback(jvmtiEnv *jvmti_env,
     JNIEnv* jni_env,
     jthread thread) {
         threadCount --;
        //  std::cout << "one thread exit...";
        // std::cout<< "current has thread: " << threadCount << std::endl;
}
